...
import { useOrderStore, useAuthStore, useFranchiseStore } from "../../store/store";
...
const Orders = () => {
  ...
  const { orders, loading, fetchMyOrders, fetchAllOrders, deleteOrder } = useOrderStore();
  ...
  const filtered = orders.filter(o =>
    ...
    (franchiseFilter === "ALL" || !isAdmin || o.franchiseId?._id === franchiseFilter || o.franchiseId === franchiseFilter) &&
    (dateFilter === "" || (o.createdAt && o.createdAt.substring(0, 10) === dateFilter))
  );
        ...
        <div className="row"><div className="col-12">
          <div className="page-title-box d-sm-flex align-items-center justify-content-between">
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                <h4 className="mb-0" style={{ fontWeight: 800, fontSize: 18, color: "#1A1A1A" }}>{isAdmin ? "All Franchise Orders" : "My Orders"}</h4>
        ...
        <div className="row"><div className="col-12"><div className="card">
          <div className="card-header">
            <div className="d-flex align-items-center justify-content-between flex-wrap gap-2">
              ...
              <button className="btn btn-sm btn-outline-secondary" onClick={() => isAdmin ? fetchAllOrders() : fetchMyOrders()} title="Refresh">
            ...
            <div className="row mb-3 g-2 align-items-center">
              ...
              <div className="col-auto">
                <div className="position-relative" style={{ width: 250 }}>
                  <input type="text" className="form-control form-control-sm" placeholder="Search orders..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
                  <i className="bx bx-search position-absolute" style={{ top: "50%", right: 12, transform: "translateY(-50%)", color: "#adb5bd" }} />
                </div>
              </div>
              <div className="col-auto ms-auto d-flex flex-wrap gap-2">
                <input type="date" className="form-control form-control-sm" style={{ width: "auto" }} value={dateFilter} onChange={e => { setDateFilter(e.target.value); setPage(1); }} title="Filter by Date" />
                {isAdmin && (
                  <select className="form-select form-select-sm" style={{ width: "auto" }} value={franchiseFilter} onChange={e => { setFranchiseFilter(e.target.value); setPage(1); }}>
                    <option value="ALL">All Franchises</option>
                    {franchises?.map(f => <option key={f._id} value={f._id}>{f.franchiseName}</option>)}
                  </select>
                )}
                <select className="form-select form-select-sm" style={{ width: "auto" }} value={orderType} onChange={e => { setOrderType(e.target.value); setPage(1); }}>
                ...
                <select className="form-select form-select-sm" style={{ width: "auto" }} value={payStatus} onChange={e => { setPayStatus(e.target.value); setPage(1); }}>
              ...
              <table className="table table-hover table-centered align-middle mb-0 text-nowrap">
                <thead className="table-light">
                  <tr>
                    ...
                    <th>Order Status</th>
                    <th>Date</th>
                    <th className="text-center" style={{ position: "sticky", right: 0, background: "#f8f9fa", zIndex: 1, borderLeft: "1px solid #e5e7eb" }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    ...
                    <tr key={row._id}>
                      ...
                      <td style={{ position: "sticky", right: 0, background: "#fff", zIndex: 1, borderLeft: "1px solid #e5e7eb" }}>
                        <div className="d-flex justify-content-center gap-2">
                          <Link to={`/store-management-orders/view/${row._id}`} state={{ rowData: row }} className="btn btn-sm btn-soft-primary" title="View" style={{ borderRadius: 8, padding: "4px 8px" }}>
                            <i className="bx bx-show font-size-16" />
                          </Link>
                          <button onClick={() => printReceipt(row, franchise?.franchise || franchise)} className="btn btn-sm btn-soft-success" title="Print Bill" style={{ borderRadius: 8, padding: "4px 8px", border: "none", cursor: "pointer" }}>
                            <i className="bx bx-printer font-size-16" />
                          </button>
                          {isAdmin && (
                            <button onClick={() => handleDelete(row._id)} disabled={deletingId === row._id} className="btn btn-sm btn-soft-danger" title="Delete Order" style={{ borderRadius: 8, padding: "4px 8px", border: "none", cursor: deletingId === row._id ? "not-allowed" : "pointer" }}>
                              {deletingId === row._id ? <i className="bx bx-loader bx-spin font-size-16" /> : <i className="bx bx-trash font-size-16" />}
                            </button>
                          )}
                        </div>
                      </td>
