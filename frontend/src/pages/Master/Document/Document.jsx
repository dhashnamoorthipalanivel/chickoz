...
const typePill = (type) => {
  const map = { LEGAL:{color:"#2563eb",bg:"rgba(37,99,235,0.08)",border:"rgba(37,99,235,0.2)"}, LICENSE:{color:"#d97706",bg:"rgba(217,119,6,0.08)",border:"rgba(217,119,6,0.2)"}, IDENTITY:{color:"#7c3aed",bg:"rgba(124,58,237,0.08)",border:"rgba(124,58,237,0.2)"}, FINANCIAL:{color:"#059669",bg:"rgba(5,150,105,0.08)",border:"rgba(5,150,105,0.2)"}, MEDIA:{color:"#F97316",bg:"rgba(249,115,22,0.09)",border:"rgba(249,115,22,0.2)"}, ADDRESS_PROOF:{color:"#6b7280",bg:"#f3f4f6",border:"#e5e7eb"} };
  const s = map[type] || { color:"#6b7280", bg:"#f3f4f6", border:"#e5e7eb" };
  return <span style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"4px 11px", borderRadius:20, fontSize:11.5, fontWeight:700, letterSpacing:0.3, color:s.color, background:s.bg, border:`1px solid ${s.border}`, whiteSpace:"nowrap" }}>{formatLabel(type)}</span>;
};

const mandatoryPill = (val) => val
  ? <span style={{ display:"inline-flex", alignItems:"center", gap:4, padding:"4px 10px", borderRadius:20, fontSize:11.5, fontWeight:700, color:"#D91E18", background:"rgba(217,30,24,0.07)", border:"1px solid rgba(217,30,24,0.2)", whiteSpace:"nowrap" }}><i className="bx bx-check-shield" style={{ fontSize:13 }}/>Yes</span>
  : <span style={{ display:"inline-flex", alignItems:"center", gap:4, padding:"4px 10px", borderRadius:20, fontSize:11.5, fontWeight:700, color:"#9ca3af", background:"#f9fafb", border:"1px solid #e5e7eb", whiteSpace:"nowrap" }}>No</span>;
...
const Document = () => {
  ...
  const filtered = documents.filter(item =>
    (item.documentName?.toLowerCase().includes(search.toLowerCase()) || item.documentType?.toLowerCase().includes(search.toLowerCase())) &&
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await deleteDocument(deleteId); await fetchDocuments();
      toast.success("Document deleted"); setShowDeleteModal(false); setDeleteId(null);
          ...
          <div className="page-title-box d-sm-flex align-items-center justify-content-between">
            <div style={{ display:"flex", alignItems:"center", gap:14 }}>
              <div style={{ width:44, height:44, borderRadius:12, background:"linear-gradient(135deg,#D91E18 0%,#F97316 100%)", boxShadow:"0 4px 14px rgba(217,30,24,0.32)", display:"flex", alignItems:"center", justifyContent:"center" }}>
                <i className="bx bx-file" style={{ color:"#fff", fontSize:22 }}/>
              </div>
              <div>
                <h4 className="mb-0" style={{ fontWeight:800, fontSize:18, color:"#1A1A1A" }}>Document Management</h4>
                <div style={{ fontSize:12, color:"#F97316", fontWeight:600, marginTop:1 }}>Masters · Document</div>
              </div>
            </div>
            <ol className="breadcrumb m-0"><li className="breadcrumb-item"><Link to="/dashboard">Dashboard</Link></li><li className="breadcrumb-item active">Document</li></ol>
          </div>
        </div></div>
        ...
        <div className="row"><div className="col-12"><div className="card">
          <div className="card-header">
            <div className="d-flex align-items-center justify-content-between flex-wrap gap-2">
              ...
              <Link to="/master-document/add" className="btn btn-sm btn-primary"><i className="bx bx-plus me-1"/>Add Document</Link>
            </div>
          </div>
          <div className="card-body">
            ...
            <div className="table-responsive" style={{ overflowX:"auto" }}>
              <table className="table table-hover table-centered align-middle mb-0 text-nowrap">
                ...
                <tbody>
                  {paged.length === 0 ? <tr><td colSpan="6" className="text-center py-5 text-muted"><i className="bx bx-search-alt display-4 d-block mb-2"/>No document records found.</td></tr>
                  : paged.map((row, i) => (
                    <tr key={row._id}>
                      <td>{(page-1)*perPage+i+1}</td>
                      <td style={{ fontWeight:600 }}>{row.documentName}</td>
                      <td>{typePill(row.documentType)}</td>
                      <td>{mandatoryPill(row.isMandatory)}</td>
                      <td>{statusPill(row.status)}</td>
                      <td><div className="d-flex justify-content-center gap-2">
                        <Link to={`/master-document/edit/${row._id}`} state={{ rowData:row }} className="ckz-action-btn ckz-action-edit" title="Edit"><i className="bx bx-edit-alt"/></Link>
