...
import { useOrderTypes } from "../../../store/store";
...
const yesNoPill = (value) => {
  const s = value
    ? { color:"#059669", bg:"rgba(5,150,105,0.08)", border:"rgba(5,150,105,0.2)", label:"Yes" }
    : { color:"#6b7280", bg:"#f3f4f6", border:"#e5e7eb", label:"No" };
  return <span style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"4px 11px", borderRadius:20, fontSize:11.5, fontWeight:700, color:s.color, background:s.bg, border:`1px solid ${s.border}`, whiteSpace:"nowrap" }}>{s.label}</span>;
};
...
const OrderType = () => {
  ...
  const { orderTypes, fetchOrderTypes, deleteOrderType } = useOrderTypes();
  useEffect(() => { fetchOrderTypes(); }, []);

  const filtered = orderTypes.filter(item =>
    (item.orderTypeName?.toLowerCase().includes(search.toLowerCase()) || item.shortName?.toLowerCase().includes(search.toLowerCase())) &&
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await deleteOrderType(deleteId); await fetchOrderTypes();
      toast.success("Order type deleted"); setShowDeleteModal(false); setDeleteId(null);
          ...
          <div className="page-title-box d-sm-flex align-items-center justify-content-between">
            <div style={{ display:"flex", alignItems:"center", gap:14 }}>
              <div style={{ width:44, height:44, borderRadius:12, background:"linear-gradient(135deg,#D91E18 0%,#F97316 100%)", boxShadow:"0 4px 14px rgba(217,30,24,0.32)", display:"flex", alignItems:"center", justifyContent:"center" }}>
                <i className="bx bx-list-ul" style={{ color:"#fff", fontSize:22 }}/>
              </div>
              <div>
                <h4 className="mb-0" style={{ fontWeight:800, fontSize:18, color:"#1A1A1A" }}>Order Type Management</h4>
                <div style={{ fontSize:12, color:"#F97316", fontWeight:600, marginTop:1 }}>Masters · Order Type</div>
              </div>
            </div>
            <ol className="breadcrumb m-0"><li className="breadcrumb-item"><Link to="/dashboard">Dashboard</Link></li><li className="breadcrumb-item active">Order Type</li></ol>
          </div>
        </div></div>
          ...
          <div className="card-header">
            <div className="d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                <h4 className="card-title mb-0">Order Type Records</h4>
              ...
              <Link to="/master-order-type/add" className="btn btn-sm btn-primary"><i className="bx bx-plus me-1"/>Add Order Type</Link>
            </div>
          </div>
          <div className="card-body">
            <div className="row mb-3 g-2 align-items-center">
              ...
              <div className="col-sm-12 col-md-5">
                <div className="position-relative">
                  <input type="text" className="form-control" placeholder="Search by name, code or short name..." value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}}/>
            ...
            <div className="table-responsive" style={{ overflowX:"auto" }}>
              <table className="table table-hover table-centered align-middle mb-0 text-nowrap">
                ...
                <tbody>
                  {paged.length === 0 ? <tr><td colSpan="6" className="text-center py-5 text-muted"><i className="bx bx-search-alt display-4 d-block mb-2"/>No order types found.</td></tr>
                  : paged.map((row, i) => (
                    <tr key={row._id}>
                      <td>{(page-1)*perPage+i+1}</td>
                      <td style={{ fontWeight:600 }}>{row.orderTypeName}</td>
                      <td><span style={{ background:"rgba(217,30,24,0.07)", color:"#D91E18", borderRadius:6, padding:"2px 8px", fontSize:12, fontWeight:700 }}>{row.shortName}</span></td>
                      <td>{yesNoPill(row.serviceChargeApplicable)}</td>
                      <td>{statusPill(row.status)}</td>
                      <td><div className="d-flex justify-content-center gap-2">
                        <Link to={`/master-order-type/edit/${row._id}`} state={{ rowData:row }} className="ckz-action-btn ckz-action-edit" title="Edit"><i className="bx bx-edit-alt"/></Link>
