...
const Vendor = () => {
  ...
  const { vendors, fetchVendors, deleteVendor } = useVendorStore();
  ...
  const filtered = (vendors || []).filter(item => {
    ...
    let sMatch = true;
    if (statusFilter !== "ALL") {
      sMatch = item.status === statusFilter;
    }
    return match && sMatch;
  });
            ...
            <div className="page-title-box d-sm-flex align-items-center justify-content-between">
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: "linear-gradient(135deg,#D91E18 0%,#F97316 100%)", boxShadow: "0 4px 14px rgba(217,30,24,0.32)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <i className="bx bx-buildings" style={{ color: "#fff", fontSize: 22 }} />
                </div>
                <div>
                  <h4 className="mb-0" style={{ fontWeight: 800, fontSize: 18, color: "#1A1A1A" }}>Vendor Management</h4>
                  <div style={{ fontSize: 12, color: "#F97316", fontWeight: 600, marginTop: 1 }}>Masters · Vendor</div>
                </div>
              </div>
            ...
            <div className="card-header">
              <div className="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <h4 className="card-title mb-0">Vendor Records</h4>
                ...
                <Link to="/master-vendor/add" className="btn btn-sm btn-primary"><i className="bx bx-plus me-1" />Add Vendor</Link>
              </div>
            </div>
            <div className="card-body">
              <div className="row mb-3 g-2 align-items-center">
                ...
                <div className="col-sm-12 col-md-4">
                  <div className="position-relative">
                    <input type="text" className="form-control" placeholder="Search by name, contact or phone..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
              ...
              <div className="table-responsive" style={{ overflowX: "auto" }}>
                <table className="table table-hover table-centered align-middle mb-0 text-nowrap">
                  ...
                  <tbody>
                    {paged.length === 0 ? (
                      <tr><td colSpan="9" className="text-center py-5 text-muted"><i className="bx bx-search-alt display-4 d-block mb-2" />No vendor records found.</td></tr>
                    ) : paged.map((row, i) => (
                      <tr key={row._id}>
                        <td>{(page - 1) * perPage + i + 1}</td>
                        <td style={{ fontWeight: 600, color: "#3b82f6" }}>{row.vendorCode || "N/A"}</td>
                        <td>{row.name}</td>
                        <td>{row.companyName || "-"}</td>
                        <td>{row.phone || "-"}</td>
                        <td>{row.email || "-"}</td>
                        <td style={{ fontFamily: "monospace", color: "#4b5563" }}>{row.gstNo || "-"}</td>
                        <td>{statusPill(row.status)}</td>
                        <td>
                          <div className="d-flex justify-content-center gap-2">
                            <Link to={`/master-vendor/edit/${row._id}`} state={{ rowData: row }} className="ckz-action-btn ckz-action-edit" title="Edit"><i className="bx bx-edit-alt" /></Link>
