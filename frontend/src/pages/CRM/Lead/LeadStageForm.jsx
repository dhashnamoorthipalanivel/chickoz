...
const resolveFranchiseCoords = async (f) => {
  if (f.latitude && f.longitude) {
    return { name: f.franchiseName || f.franchiseId || "Franchise", lat: Number(f.latitude), lng: Number(f.longitude) };
  }
  ...
  // 1. Check franchise name first
  for (const [key, coords] of Object.entries(KNOWN_CITY_COORDS)) {
    if (nameText.includes(key)) {
      return { name: f.franchiseName || f.franchiseId || f.location || "Franchise", lat: coords[0], lng: coords[1] };
    }
  }

  // 2. Check address
  for (const [key, coords] of Object.entries(KNOWN_CITY_COORDS)) {
    if (addrText.includes(key)) {
      return { name: f.franchiseName || f.franchiseId || f.location || "Franchise", lat: coords[0], lng: coords[1] };
    }
  }
  ...
  // 4. Try geocoding with OpenStreetMap Nominatim
  const queryStr = f.location || f.address;
  if (queryStr) {
    const geo = await geocodeStr(queryStr);
    if (geo) {
      return { name: f.franchiseName || f.franchiseId || f.location || "Franchise", ...geo };
    }
  }

  return {
    name: f.franchiseName || f.franchiseId || "Franchise",
    lat: 11.4428,
    lng: 77.7126,
  };
};
  ...
  useEffect(() => {
    ...
    (async () => {
      const results = await Promise.all(
        franchises.map(async (f, idx) => {
          return await resolveFranchiseCoords(f, idx);
        })
      );
